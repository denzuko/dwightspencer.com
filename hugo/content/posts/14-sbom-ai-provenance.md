...
A Software Bill of Materials answers three questions: what is in this software, where did
...
## What SBOM tooling sees and does not see

SBOM generation tools running in your CI/CD pipeline will accurately enumerate every
package your AI-generated code depends on. They will not record that the function calling
...
created with AI assistance. Most of those organisations are generating SBOMs that say
...
in the defence industrial base it is an active compliance problem with a hard deadline.
...
If AI-generated code is part of your software environment and you cannot document the
provenance and review history of that code, you have a gap that self-assessments did not
surface.

## Three controls worth implementing now

**Attribution at the commit level.** Git already records author, timestamp, and commit
message. A policy requiring commits containing AI-generated code to be labelled as such
...
functional. organisations that start generating AI-BOMs now will have a year of data
...
organisations already have branch protection rules. Adding an AI-assist check to the
...
that AI writes code — it is that the controls applied to every other input to your build
process have not been applied to AI input.
...
do not find out that "we use Copilot" is not an answer to "show me your CUI handling
documentation."
