...
<p>That tax comes from the grammar itself in all four cases, not from the vendor or framework sitting on top of it. A model cannot optimise the overhead away, and it persists whether or not the model needs it to understand the operation.</p>
...
<li><strong>Macro amortisation only exists where the grammar is extensible.</strong> Every call site of <code>defpipe</code> pays a fraction of the full expansion. Multiply that by the number of times a pattern like this appears across a codebase being fed into a context window as few-shot examples or RAG snippets, and the savings compound linearly with call-site count. Naming discipline alone does not replicate this in the ALGOL family, which has no mechanism to name a structural pattern and let the compiler expand it back out.</li>
</ul>

<p>The whitespace and naming compression above is an injection-time transform, and it has no place in a codebase anyone has to maintain. Macro-driven compression is a different story. Dense, macro-heavy Lisp is a documented production tradition, most associated with the "Let Over Lambda" school of macrology. The pattern applies to systems well outside this argument, and token golf did not invent it. What changes for context-window injection is the scale of the payoff. A macro that saves a maintainer a few lines of typing saves a model dozens of tokens at every call site, and that gap compounds across a system prompt or RAG corpus in a way it never does across a single human-read file.</p>
