...
<li><strong>node_exporter</strong> — host-level metrics: CPU, memory, disk I/O, network
interfaces, ZFS pool health. Run one per node as a quadlet under its
own system account.</li>
<li><strong>cAdvisor</strong> — per-container resource metrics for every running Podman
container. Pairs with node_exporter so you can correlate host pressure
with the specific service causing it.</li>
